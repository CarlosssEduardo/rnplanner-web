...
const VisitaScreen = () => {
  ...
  const executarFinalizacao = async () => {
    try {
      ...
      setModal({
        visible: true,
        title: isModoPendencias
          ? "Alterações Salvas!"
          : "Visita Finalizada! ✅",
        message: isModoPendencias
          ? "Os acordos foram atualizados com sucesso."
  ...
  const alternarStatusPendencia = (id) => {
    setPendencias(
      pendencias.map((p) =>
        p.id === id
          ? { ...p, status: p.status === "PENDENTE" ? "RESOLVIDO" : "PENDENTE" }
          : p,
      ),
    );
  };
    ...
    <div className="contadorCard" style={{ borderLeft: `6px solid ${cor}` }}>
      ...
      <div className="contadorControls">
        ...
        <input
          ...
          placeholder="0"
          onChange={(e) =>
            setValor(e.target.value === "" ? 0 : parseInt(e.target.value))
          }
  ...
  return (
    <div className="safeAreaVisita">
      {modal.visible && (
        <div className="modalOverlayPro">
          <div className="modalBoxPro">
            <div className="modalHeaderPro">
              <h3>{modal.title}</h3>
            </div>
            <div className="modalBodyPro">
              <p>{modal.message}</p>
            </div>
      ...
      <div className="headerVisita">
        ...
        <span className="headerTitleVisita">
          {isModoPendencias ? "Gestão de Acordos" : "Atendimento"}
        </span>
      </div>
      ...
      <div className="contentVisita">
        ...
        {!isModoPendencias && (
          <div className="sectionVisita">
            ...
            {renderContadorPro(
              "🏷️ Ofertas e Campanhas",
              qtdOfertas,
              setQtdOfertas,
              COLORS.OFERTA_COLOR,
            )}
            {renderContadorPro(
              "🎯 Missões de Portfólio",
              qtdMissoes,
              setQtdMissoes,
              COLORS.MISSAO_COLOR,
            )}
        ...
        <div className="sectionVisita">
          ...
          <span className="sectionSubTitleVisita">
            Acompanhe o que precisa ser resolvido no cliente.
          </span>
          ...
          <div className="novaPendenciaRow">
            <input
              ...
              placeholder="Ex: Verificar boleto..."
              value={novaPendencia}
              onChange={(e) => setNovaPendencia(e.target.value)}
              onKeyDown={(e) => e.key === "Enter" && adicionarPendencia()}
            />
            <button className="btnAddPendencia" onClick={adicionarPendencia}>
              <span className="btnAddPendenciaText">ADD</span>
            </button>
          </div>
          ...
          <div className="pendenciasList">
            {pendencias.map((pend) => (
              <div
                key={pend.id}
                className={`pendenciaCard ${pend.status === "RESOLVIDO" ? "pendenciaCardResolvido" : ""}`}
              >
                <div
                  className="pendenciaContent"
                  onClick={() => alternarStatusPendencia(pend.id)}
                >
                  <div
                    className={`statusIndicator ${pend.status === "RESOLVIDO" ? "statusResolvido" : "statusPendente"}`}
                  />
                  <div className="pendenciaTextWrapper">
                    <span
                      className={`pendenciaTexto ${pend.status === "RESOLVIDO" ? "pendenciaTextoResolvido" : ""}`}
                    >
                      {pend.texto}
                    </span>
                ...
                <button
                  className="btnApagarPendencia"
                  onClick={() => removerPendencia(pend.id)}
                >
            ...
            {pendencias.length === 0 && (
              <span className="emptyPendencias">
                Nenhum acordo registado neste PDV.
              </span>
            )}
          </div>
        </div>
        ...
        <button
          className={`finishButton ${isSaving ? "finishButtonDisabled" : ""}`}
