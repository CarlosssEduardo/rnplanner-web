...
const HomeScreen = () => {
  ...
  const carregarDados = async () => {
    try {
      ...
      // 🔥 MÁGICA 1: O Fallback Mapping. Traduzindo o JSON do Java pro React
      if (dadosDash) {
        setDashboard({
          ...dadosDash,
          tasksCompraTotal: dadosDash.compra || dadosDash.tasksCompraTotal || 0,
          tasksCervejaTotal: dadosDash.cerveja || dadosDash.tasksCervejaTotal || 0,
          tasksNabTotal: dadosDash.nab || dadosDash.tasksNabTotal || 0,
          tasksMktTotal: dadosDash.mkt || dadosDash.tasksMktTotal || 0,
          compradoresTotal: dadosDash.compradores || dadosDash.compradoresTotal || 0,
          pdvsVisitadosIds: dadosDash.ids || dadosDash.pdvsVisitadosIds || []
        });
      }
            ...
            {pdvsFiltrados.map(pdv => {
                ...
                return (
                    <div key={pdv.id} className={`cardPdv ${isConcluido ? 'cardPdvConcluido' : ''}`} style={pdv.rkg && pdv.rkg <= 5 ? { borderLeft: '6px solid #0d6efd' } : {}} onClick={() => handlePressPdv(pdv)}>
                        {/* 🔥 MÁGICA 2: Mostrando o ID Ambev verdadeiro no card para o entregador */}
                        <div className="cardHeader"><span className="cardTitle">{pdv.nome}</span><span className="codigoText">#{pdv.idUnico || pdv.id}</span></div>
